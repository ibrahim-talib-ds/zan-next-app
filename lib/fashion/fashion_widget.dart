...
class FashionWidget extends StatefulWidget {
  ...
  static String routePath = '/fashion';
...
class _FashionWidgetState extends State<FashionWidget> {
  late FashionModel _model;

  final scaffoldKey = GlobalKey<ScaffoldState>();

  // ------------------------------------------------------------
  // Every Fashion subcategory. Adding a new one = one line.
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => FashionModel());
    logFirebaseEvent('screen_view', parameters: {'screen_name': 'Fashion'});
  ...
  Widget _subTile(BuildContext context, _Sub sub) {
    ...
    return InkWell(
      ...
      onTap: () async {
        logFirebaseEvent('FASHION_subcategory_ON_TAP');

        // Save the chosen subcategory, then go straight to Add Product.
        ...
        try {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Cannot open Add Product')),
          );
        }
      },
      child: Container(
